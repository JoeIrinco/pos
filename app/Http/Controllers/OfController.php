...
class OfController extends Controller
{
    ...
    {
        $store_orders=DB::table('store_orders')->where('transaction_type','ORDER FORM')->where('transaction_id',$transaction_id)->first();
        if($store_orders != ''){
            ...
            return view('store.of', compact('store_orders','store_items','getpurchasenumber','gettotalpayment'));
        }
