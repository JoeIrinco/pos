...
class StorePurchaseOrderListController extends Controller
{
    /**
    ...
    {
        return view('store.purchase-order-list');
    }

        //get datatable items
    public function storegetPoList(Request $request)
    {   
        $columns = array(
        ...
        $totalitems = DB::table('store_purchase_orders')->get()->count();
         if($request->input('search.value')){
            ...
            $auth_id =Auth::id();
            $items = StorePurchaseOrder::select("*")/* ->where('user_id', $auth_id)
            ->where('orders_id',null)
             */->where(function($query) use ($searchByUser){
                   $query->where('id','LIKE',"%".$searchByUser."%")
                   ->orWhere('po_no','LIKE',"%".$searchByUser."%")
                   ->orWhere('date','LIKE',"%".$searchByUser."%")
                   ->orWhere('total_price','LIKE',"%".$searchByUser."%")
                   ->orWhere('encoder','LIKE',"%".$searchByUser."%")
            ...
            $auth_id =Auth::id();
            $items = DB::table('store_purchase_orders')/* ->where('user_id', $auth_id)
            ->where('orders_id', null) */
            ->offset($start)
            ->limit($limit)
            ->orderBy($order, $dir)
        ...
        if($items){
            foreach ($items as $item)
            {
                ...
                $nestedData['total_price'] = number_format($item->total_price);
